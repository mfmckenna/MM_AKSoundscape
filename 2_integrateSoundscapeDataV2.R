...
#- time zone is likely GMT- confirmed by Martin
...
# 4) Tide data- station 9468756 https://tidesandcurrents.noaa.gov/map/index.html?id=9468756
# - hourly water levels, predicted and verified
# - calculate the difference from previous hour and can determine magnitude and direction
...
#COMPARING HOURLY SAMPLES- what is the start time of the samples?
##deployments 2015.2, 2015-07-07
...
#deployments 2015.11, 2015-11-28
...
multmerge = function(path){
  filenames = list.files(path=path, pattern = "CO-OPS_9468756_", full.names=TRUE, recursive = TRUE)
  rbindlist(lapply(filenames, fread))
}
# list.files(path=dir, pattern = "CO-OPS_8443970_", full.names=TRUE, recursive = TRUE)
TIDE <- multmerge("D:\\RESEARCH\\COA_Projects\\2020_COA_WCS\\data")
...
#-----------------------------------------------------------
dataWind$dataTime =dataWind$dataTime = force_tz( dataWind$DateF, tzone="GMT")
...
#-----------------------------------------------------------
#3) COMBINE TIDE WITH Species, SPL,  AIS, Wind
#-----------------------------------------------------------
dataTide$DateF = force_tz( dataTide$DateF, tzone="GMT") #dataTide$DateF[1:5]
dataTide$changeWL = as.numeric( as.character(dataTide$changeWL)) #plot(dataTide$changeWL)

dataSpAShWe$WaterLevelDiff = NA
dataSpAShWe$tideDateTime = NA

for (ii in 1:nrow(dataSpAShWe)){ #{ # ii =  4753 ii = 3337 new deployment on 10-14-2015, dataSpAShWe$dataTime[3331:3350]
  
  hr  = format(dataSpAShWe$dataTime[ii], "%H")
  shr =  force_tz( as.POSIXct( paste0( dataSpAShWe$dateStart[ii], " ", hr ,":00:00") , 
                        format = "%Y-%m-%d %H:%M:%S"),tzone="GMT")
  tmp1 = dataTide[dataTide$DateF==shr, ]
  
  if (nrow(tmp1) == 1 ){
    dataSpAShWe$WaterLevelDiff[ii] = tmp1$changeWL
    dataSpAShWe$tideDateTime[ii] = as.character( tmp1$DateF)
    cat(as.character( dataSpAShWe$dataTime[ii] ), " matches ", as.character(tmp1$DateF ),"\n" )
  } else {
    cat(as.character( dataSpAShWe$dataTime[ii] ), " No matches ","\n" )
  }
  
  rm(tmp1,hr,shr)
}
# length(missingRows) + nrow(tideInfo)

dataSpAShWeTi = dataSpAShWe
#-----------------------------------------------------------
# SAVE OUT BIG MATRIX!!!
#-----------------------------------------------------------
ice = read.csv("D:\\RESEARCH\\COA_Projects\\2020_COA_WCS\\data\\recordings_ice_concentration.csv")
ice = ice[complete.cases(ice), ]
iceGambell = ice[ice$deployment_id == "2015.2" | ice$deployment_id == "2015.11",]
#add ice to big matrix-- use id
iceCom = NULL
for (ii in 1:nrow(dataSpAShWeTi)){
  idx = match(dataSpAShWeTi$recording_id[ii], iceGambell$id_recording )
  iceCom = rbind(iceCom, iceGambell [idx,])}

dataSpAShWeTiIce = cbind(dataSpAShWeTi,iceCom)

#-----------------------------------------------------------
# SAVE OUT BIG MATRIX!!!
#-----------------------------------------------------------
#save(dataSpAShWe,     file = "D:\\RESEARCH\\COA_Projects\\2020_COA_WCS\\data\\dataSpAShWeTi")
save(dataSpAShWeTiIce, file = "D:\\RESEARCH\\COA_Projects\\2020_COA_WCS\\data\\dataSpAShWeTiIce")
